...
export function MultiSelect({ label, options, value, onChange, formatOption = (option) => option, locale = "de", compact = false, disabled = false }: { label: string; options: string[]; value: string[]; onChange: (value: string[]) => void; formatOption?: (option: string) => string; locale?: Locale; compact?: boolean; disabled?: boolean }) {
  return <Popover><PopoverTrigger asChild><Button type="button" variant="outline" size={compact ? "sm" : "default"} disabled={disabled} className="min-w-0 w-full shrink justify-between overflow-hidden"><span className="min-w-0 truncate">{label}</span><ChevronsUpDownIcon className="shrink-0" data-icon="inline-end" /></Button></PopoverTrigger>
