...
import { teamProjects, type HumanAccessSourceStatus, type TeamMember } from "@/types";
...
export function EmployeeManagement({ locale }: { locale: Locale }) {
  ...
  const [selected, setSelected] = useState<Array<(typeof teamProjects)[number]>>([]);
  ...
  return <Dialog open={open} onOpenChange={(value) => { setOpen(value); setEnrollmentCode(""); if (value) void load(); }}>
    ...
    <DialogContent className="max-h-[90vh] min-w-0 overflow-y-auto sm:max-w-2xl">
      ...
      <FieldGroup className="min-w-0">
        ...
        <div className="flex flex-wrap gap-4">{teamProjects.map((project) => <label key={project} className="flex items-center gap-2 text-sm"><Checkbox checked={selected.includes(project)} onCheckedChange={(checked) => setSelected((current) => checked ? [...current, project] : current.filter((value) => value !== project))} />{project.toUpperCase()}</label>)}</div>
