...
export function TaxonomySettings({ taxonomies, locale, onSaved }: { taxonomies: Taxonomies; locale: Locale; onSaved: (value: Taxonomies) => void }) {
  ...
  return <Sheet open={open} onOpenChange={setOpen}><SheetTrigger asChild><Button variant="outline"><SettingsIcon data-icon="inline-start" />{locale === "de" ? "Auswahllisten" : "Lists"}</Button></SheetTrigger><SheetContent className="w-full overflow-y-auto"><SheetHeader><SheetTitle>{locale === "de" ? "Auswahllisten" : "Lists"}</SheetTitle><SheetDescription>{locale === "de" ? "Ein Eintrag pro Zeile. Die 16 ORISO-Themen sind als stabile Schlüssel hinterlegt." : "One entry per line. The 16 ORISO topics are stored as stable keys."}</SheetDescription></SheetHeader><FieldGroup className="px-4">{(["roles","topics","conversationTypes"] as const).map((kind) => <Field key={kind}><FieldLabel htmlFor={`taxonomy-${kind}`}>{kind === "roles" ? (locale === "de" ? "Rollen" : "Roles") : kind === "topics" ? (locale === "de" ? "Themengebiete" : "Topics") : (locale === "de" ? "Konversationstypen" : "Conversation types")}</FieldLabel><Textarea id={`taxonomy-${kind}`} rows={7} value={draft[kind].join("\n")} onChange={(e) => edit(kind, e.target.value)} /></Field>)}</FieldGroup><SheetFooter><Button onClick={save}>{locale === "de" ? "Speichern" : "Save"}</Button></SheetFooter></SheetContent></Sheet>;
}
