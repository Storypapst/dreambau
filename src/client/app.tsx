...
export function App() {
  ...
  useEffect(() => { if (session?.authenticated && session.method !== "password-bootstrap") { setLoadError(false); Promise.all([api<AccountView[]>("/accounts"), api<Taxonomies>("/taxonomies")]).then(([a,t]) => { setAccounts(a); setTaxonomies(t); }).catch(() => setLoadError(true)); } }, [session]);
