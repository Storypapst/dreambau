...
export function App() {
  ...
  useEffect(() => { if (session?.authenticated && session.method === "passkey") { setLoadError(false); Promise.all([api<AccountView[]>("/accounts"), api<Taxonomies>("/taxonomies"), api<HumanUser>("/auth/me")]).then(([a,t,user]) => { setAccounts(a); setTaxonomies(t); setCurrentUser(user); }).catch(() => setLoadError(true)); } }, [session]);
  ...
  return <TooltipProvider>{session?.authenticated && session.method !== "passkey" ? <PasskeyEnrollment locale={locale} onComplete={refreshSession} /> : session?.authenticated && accounts.length && currentUser ? <AccountDirectory initialAccounts={accounts} initialTaxonomies={taxonomies} locale={locale} onLocaleChange={setLocale} isAdmin={currentUser.role === "admin"} onLogout={() => { setSession({ authenticated: false }); setAccounts([]); setCurrentUser(null); }} /> : session?.authenticated === false ? <LoginForm locale={locale} onLocaleChange={setLocale} onAuthenticated={refreshSession} /> : loadError ? <main className="grid min-h-screen place-items-center p-6"><Alert variant="destructive" className="max-w-lg"><AlertTitle>{locale === "de" ? "Konten konnten nicht geladen werden" : "Accounts could not be loaded"}</AlertTitle><AlertDescription>{locale === "de" ? "Bitte Seite neu laden oder den Serverstatus prüfen." : "Reload the page or check the server status."}</AlertDescription></Alert></main> : <main className="mx-auto flex min-h-screen max-w-5xl flex-col gap-4 p-8"><Skeleton className="h-16 w-2/3" /><Skeleton className="h-12 w-full" /><Skeleton className="h-72 w-full" /></main>}<Toaster /></TooltipProvider>;
}
