...
export function generateMarkdown(accounts: AccountView[], taxonomies: Taxonomies) {
  ...
  for (const domain of domains) {
    ...
    for (const account of accounts.filter((item) => item.domain === domain).sort((a,b) => a.email.localeCompare(b.email))) {
      ...
      lines.push(`| ${escape(account.displayName)} | ${escape(account.email)} | ${escape(account.password)} | ${encryption} | ${escape(m.shippedVersion)} | ${m.lifecycleStatus} | ${m.project} | ${escape(m.roles.join(", "))} | ${escape(m.topics.join(", "))} | ${escape(m.conversationTypes.join(", "))} | ${m.fixtureQuality} | ${m.sampleFileCount} | ${escape(m.notes)} |`);
    }
    lines.push("");
  }
  return `${lines.join("\n")}\n`;
