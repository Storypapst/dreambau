...
vi.mock("@/components/account-directory", () => ({
  AccountDirectory: ({ initialAccounts }: { initialAccounts: unknown[] }) => <div data-testid="directory">accounts:{initialAccounts.length}</div>
}));
...
describe("App authenticated loading", () => {
  ...
  it("renders the account directory after a successful empty account response", async () => {
    ...
    await vi.waitFor(() => expect(container.querySelector('[data-testid="directory"]')?.textContent).toBe("accounts:0"));
  });
});
