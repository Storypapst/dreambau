...
describe("Kubernetes Infisical bootstrap", () => {
  it("mounts only client credentials from a dedicated Secret and keeps project IDs non-secret", () => {
    ...
    expect(deployment).toContain("INFISICAL_WRITER_CLIENT_ID_FILE, value: /run/secrets/infisical-writer/client-id");
    expect(deployment).toContain("INFISICAL_WRITER_CLIENT_SECRET_FILE, value: /run/secrets/infisical-writer/client-secret");
    expect(deployment).toContain("secretName: test-access-infisical-writer");
    ...
    expect(entries.find((entry) => entry.name === "INFISICAL_WRITER_CLIENT_SECRET")).toBeUndefined();
